...
static bool convert_stat(struct stat *out, smb2_stat_64 *in)
{
    ...
    {
    case SMB2_TYPE_LINK:
        out->st_mode = S_IFLNK | SMB_FILE_READ;
        if (!read_only)
            out->st_mode |= SMB_FILE_WRITE;
        break;
    case SMB2_TYPE_FILE:
        out->st_mode = S_IFREG | SMB_FILE_READ;
        if (!read_only)
            out->st_mode |= SMB_FILE_WRITE;
        break;
    case SMB2_TYPE_DIRECTORY:
        out->st_mode = S_IFDIR | SMB_DIR_READ;
        if (!read_only)
                            ...
                            struct fuse_file_info *fi )
{
    ...
    if( strcmp(path, "/") == 0 ) 
    {
        stbuf->st_mode = S_IFDIR | SMB_DIR_READ | SMB_DIR_WRITE;
        stbuf->st_nlink = 2;
                         ...
                         struct fuse_file_info *fi )
{
    ...
    printf( "%s smb2_read @ %li (%p, %zi)", CODE_REF, offset, buf, size );
